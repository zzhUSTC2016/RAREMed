...
from utils.layers import SelfAttend, GraphConvolution

class COGNet(nn.Module):
    """在CopyDrug_batch基础上将medication的encode部分修改为transformer encoder"""
    def __init__(self, args, voc_size, ehr_adj, ddi_adj, ddi_mask_H):
    ...
    def decode(self, input_medications, input_disease_embedding, input_proc_embedding, last_medication_embedding, last_medications, cross_visit_scores,
        ...
        input_medication_embs = input_medication_embs.to(self.device) # added0403
...
class GCN(nn.Module):
    def __init__(self, voc_size, emb_dim, ehr_adj, ddi_adj, device=None):
