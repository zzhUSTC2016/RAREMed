...
from utils.util import ddi_rate_score
import torch
import numpy as np
import copy
import random
...
class Beam(object):
    ...
    def __init__(self, size, PAD_TOKEN, BOS_TOKEN, EOS_TOKEN, ddi_adj, device):
        ...
        self.scores = self.tt.FloatTensor(size).zero_().to(device=device)
        self.all_scores = []
    ...
    def advance(self, word_lk):
        ...
        cur_output = self.get_current_state(sort=False).to(self.device) # add 0403 

        active_scores = self.scores[active_beam_idx]
        end_scores = self.scores[end_beam_idx].to(self.device) # add 0403
        ...
        flat_beam_lk = beam_lk.view(-1).to(self.device) # add 0403
